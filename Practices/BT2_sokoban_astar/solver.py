...
def uniformCostSearch(gameState):                   # Hàm nhận tham số gameState là trạng thái khởi đầu của trò chơi
    ...
    temp = []                                       # Lưu danh sách các hành động từ trạng thái bắt đầu đến trạng thái kết thúc
    count = 0                                       # Khởi tạo biến count để đếm số nút được mở rộng
    while frontier:                                 # Kiểm tra frontier có rỗng hay không? Nếu không rỗng thì vào vòng lặp để xử lý
        node = frontier.pop()                       # Lấy ra phần tử đầu tiên bên trái của priority queue frontier và xóa nó.
        count += 1                                  # 1 lần đi qua 1 nút thì count tăng lên 1
        ...
        if node[-1] not in exploredSet:             # Kiểm tra node[-1] có nằm trong exploredSet hay không? (Player đã đi qua hay chưa?)
            ...
            for action in legalActions(node[-1][0], node[-1][1]): # Xét tất cả các hành động hợp lệ có thể thực hiện từ hành động hiện tại
                ...
                if isFailed(newPosBox):             # Kiểm tra vị trí mới của Box có hợp lệ hay không?
                    continue                        # Nếu Box ở vị trí không hợp lệ, bỏ qua và chuyển sang action khác.
                Cost = cost(node_action[1:] + [action[-1]])             # Tính chi phí hiện tại dựa trên số hành động đã thực hiện.
    ...
    return temp, count, runtime                     # Trả về danh sách các hành động đến trạng thái mục tiêu của thuật toán UCS
                                                    # Số nút được mở rộng, thời gian chạy
def heuristicManhattan(posPlayer, posBox):
...
def heuristicEuclid(posPlayer, posBox):
    """A heuristic function to calculate the overall Euclidean distance between the boxes and the goals"""
    distance = 0
    completes = set(posGoals) & set(posBox)
    sortposBox = list(set(posBox).difference(completes))
    sortposGoals = list(set(posGoals).difference(completes))
...
def aStarSearch(gameState, heuristic = heuristicEuclid):
    """Implement aStarSearch approach"""
    time_start =  time.time()
    beginBox = PosOfBoxes(gameState)            # Vị trí ban đầu của các BOX trong bản đồ
    beginPlayer = PosOfPlayer(gameState)        # Vị trí ban đầu của Player trong bản đồ
    start_state = (beginPlayer, beginBox)       # Trạng thái bắt đầu gồm: vị trí bắt đầu của Player và các Box
    frontier = PriorityQueue()                  # Hàng đợi Priority Queue frontier để lưu trữ các trạng thái của trò chơi, sắp xếp theo chi phí
    frontier.push([start_state], heuristic(beginPlayer, beginBox))  # Thêm startState vào "frontier", chi phí ước lượng từ vị trí bắt đầu 
                                                                    # đến vị trí mục tiêu dựa trên vị trí của các hộp và các mục tiêu.
    exploredSet = set()                         # Dùng set() để lưu các hành động mà Player đã đi qua và tránh trùng lặp
    actions = PriorityQueue()                   # Hàng đợi Priority queue actions để lưu trữ các hành động, sắp xếp theo chi phí
    actions.push([0], heuristic(beginPlayer, start_state[1]))
    temp = []                                   # Lưu danh sách các hành động từ trạng thái bắt đầu đến trạng thái kết thúc
    count = 0                                   # Khởi tạo biến count để đếm số nút được mở rộng
    while len(frontier.Heap) > 0:               # Kiểm tra frontier có rỗng hay không? Nếu không rỗng thì vào vòng lặp để xử lý
        node = frontier.pop()                   # Lấy ra phần tử đầu tiên bên trái của priority queue frontier và xóa nó
        count += 1                              # 1 lần đi qua 1 nút thì count tăng lên 1
        node_action = actions.pop()             # Lấy ra hành động đầu tiên bên trái của Priority Queue actions và xóa nó
        if isEndState(node[-1][-1]):            # Kiểm tra xem đây có phải trạng thái kết thúc hay không?
        ...
        if node[-1] not in exploredSet:         # Kiểm tra node[-1] có nằm trong exploredSet hay không? (Player đã đi qua hay chưa?)
            ...
            for action in legalActions(node[-1][0], node[-1][1]):  # try every possible actions
                ...
                if isFailed(newPosBox):         # Xét tất cả các hành động hợp lệ có thể thực hiện từ hành động hiện tại
                    continue                    # Nếu Box ở vị trí không hợp lệ, bỏ qua và chuyển sang action khác
                Cost = cost(node_action[1:] + [action[-1]])         # Tính chi phí hiện tại dựa trên số hành động đã thực hiện
                Heuristic = heuristic(newPosPlayer, newPosBox)      # Tính chi phí ước tính (khoảng cách) giữa các hộp và mục tiêu.
                frontier.push(node + [(newPosPlayer, newPosBox)], Heuristic + Cost)     # Thêm trạng thái mới vào frontier với chi phí tương ứng
                                                                                        # Chi phí được tính dựa trên số hành động đã thực hiện từ trạng thái ban đầu đến trạng thái hiện tại
                                                                                        # Cộng với chi phí ước tính giữa các hộp và mục tiêu
                actions.push(node_action + [action[-1]], Heuristic + Cost)              # Thêm hành động mới vào actions với chi phí tương ứng
    time_end =  time.time()
    runtime = time_end - time_start
    return temp, count, runtime                 # Trả về danh sách các hành động đến trạng thái mục tiêu của thuật toán UCS
                                                # Số nút được mở rộng, thời gian chạy
...
def get_move(layout, player_pos, method):
    ...
    if method == 'dfs':
        ...
        result, count, runtime = uniformCostSearch(gameState)
    elif method == 'astar':
        result, count, runtime = aStarSearch(gameState)        
    else:
        raise ValueError('Invalid method.')
    #time_end=time.time()
    #print('Runtime of %s: %.2f second.' %(method, time_end-time_start))
    print(f'Runtime of {method}: {runtime:.3f} seconds.')
    print('Số nút được mở:', count)
    print(result)
    print(len(result))
    return result
